...
function Projects() {
  return (
    ...
    >
      <p className="text-center md:text-[60px] sm:text-[10px] xs:text-[25px] lg:text-[40px] font-black" id="project">
        Project
      </p>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 mx-auto place-items-center gap-2 rounded-lg mt-8 xs:mt-3">
        {projects.slice(0, 3).map((pro,index) => (
          ...
          <Tilt
            ...
            className={`max-w-[370px] md:max-w-[320px] xs:max-w-[290px] border border-[#1d293a] hover:border-[#464c6a] rounded-[20px] ${index === 2 ? "lg:block hidden":""} ${index === 1 ? "hidden md:block":""}`}
          >
      ...
      <div className="cursor-pointer mx-auto font-bold text-gray-200 mt-5 md:w-auto md:p-5 xs:p-3 border border-purple-400 rounded-xl md:text-xl xs:text-sm">
        <Link to="/project/all" onClick={handleClick}>view more</Link>
      </div>
</div>
    </motion.div>
  );
